...
import 'package:exchangily_mobile_wallet/src/environments/environment.dart';
...
// getLtcTransactionStatus
  Future getLtcTransactionStatus(String txid) async {
    ...
    // ! test if this new URI format works fine
    Uri url = Uri.dataFromString(ltcBaseUrl + 'gettransactionjson/' + txid);

    try {
      response = await httpClient.get(url);
    } catch (e) {
      debugPrint('getLtcTransactionStatus CATCH: $e');
    }
...
// getLtcBalanceByAddress
  Future getLtcBalanceByAddress(String address) async {
    ...
    try {
      // ! test if this new URI format works fine
      var response = await httpClient.get(Uri.dataFromString(url));
